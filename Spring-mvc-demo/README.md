...
- Apache Tomcat
- Eclipse (Java EE version) (intellij idea in your case)
...
- delete any autogenerated index.jsp and applicationContext.xml files
    - else it'll try to run the below and you waste time trying to figure whatever the f this is: 
 ```jsp
...
####Refert to Weblogic setup notes at this point
...
- Settings->Application Servers-> + ->Tomcat
    - In Deployment tag, choose artificat that's autogenerated. 
...
- For Weblogic (based on your experimentation):
    ...
    - You now know how to create a domain but you're worried it'll mess SMS
 
- Once you get working in Tomcat, read the log output to get an idea of what Tomcat is doing

###### S1 Section 12, Lecture 106 - FAQ: HELP! My Spring MVC Controller is not working. What to do?
QUESTION: Spring MVC Controller is not working. What to do? 
ANSWER: Make sure you are accessing the correct URL
...
QUESTION: Can't start Tomcat - Ports are in use

!()[]

ANSWER: 
1a) Start and stop Tomcat as a Windows service through the control panel (Apache Tomcat)
2a) Free port which is already used to run tomcat server
    - For example, suppose 8080 port is used, we need to free 8080 to run tomcat
    - Step 1: 
        - In CMD: 
            - >$C:\Users\username>netstat -o -n -a | findstr 0.0:8080
            - >TCP 0.0.0.0:3000 0.0.0.0:0 LISTENING 3116
            - Now can see that LISTENING port is 3116 for 8080
                - need to kill 3116
    - Step 2: 
        - >$C:\Users\username\taskkill /F /PID 3116
    - Step 3: 
        - restart the server

###### S1 Section 12, Lecture 108 - FAQ: How does Component Scan Work - Your Package Names are Different! 
QUESTION
- How does Component scan work in this eg? you have different package names
You listed the component scan package as com.luv2code.springdemo
But our MVC controllers are defined in com.luv2code.springdemo.mvc
ANSWER
- For the spring attibute: base-package="com.luv2code.springdemo"
...
- The packages com.luv2code.springdemo.mvc is a sub package because of naming structure, just like folders on a file system. 
- As a result, it will be included in teh scan. 

###### S1 Section 12, Lecture 109 - Reading HTML Form Data - Overview
**High Level View**
![highlevelview]()
- Will have a form that prompts the user for their name. 
    - once they hit submit, it will go through Spring MVC app and then show a confirmation page with their name
**Application Flow**
![showForm]()
- In the browser, they'll go through our website in /showform
    - this will go through our HelloWorld Controller and the main purpose to to just show the JSP form
![processForm]()
- Then, once we have the form displayed where information is entered and they hit submit, 
    - it gets passed into /processForm request mapping
    - go back into the Hello World Controller and for that mapping, we'll pass back a confirmation page gives that name plugged in 
![keypoint]()
- Key point: Using one hello world controller but two request mappings
...
```
- Annotate spring controller with @controller for component scan
- To show intial form, using /showform to return hello-world-form
    - In the background, spring will add on prefix and suffix in configuration file to get: 
        - WEB-INF/view/hello-world-form.jsp
- The second method is for processing the form
    - this is the confirmation page that will display the student name
- The method names could be anything. We just need to map the paths to the actual methods
